...
def generate_model_patient_data(shape, scale, alpha, beta, num_patients_per_model, num_months_per_patient):
    ...
    counts are generated by a negative binomial distribution with bayesian priors on both parameters, 

    resulting in four parameters as inputs.

    Inputs:

        1) shape:

            (float) - the first parameter for the bayesian negative binomial model
                
        2) scale:

            (float) - the second parameter for the bayesian negative binomial model
                
        3) alpha:

            (float) - the third parameter for the bayesian negative binomial model
                
        4) beta:

            (float) - the fourth parameter for the bayesian negative binomial model
